...
const FIELDS = [
  { Icon: PhoneIncoming, label: "Eingang", value: "Anruf · heute 22:47" },
  ...
  { Icon: Send, label: "Übergabe", value: "E-Mail + WhatsApp an die Bereitschaft · 22:49", ok: true },
];
    ...
    <section id="beispiel" className="relative bg-paper2 py-16 lg:py-24">
      <div className="mx-auto max-w-[1200px] px-6 lg:px-12">
        ...
        <motion.div
          ...
          className="work-paper mx-auto mt-10 max-w-[720px] rounded-[6px] lg:mt-12"
        >
            ...
            {FIELDS.map((f) => (
              <div key={f.label} className="grid gap-1 border-t border-dashed border-[#D9D8CF] py-3.5 first:border-t-0 sm:grid-cols-[160px_1fr] sm:items-baseline sm:gap-4">
                ...
                {f.urgent ? (
                  ...
                  <span className="inline-flex w-fit items-center gap-1.5 rounded-full border border-[#BED2C1] bg-[#EAF0E8] px-2.5 py-1 text-[12px] font-semibold text-leaf">
                    <Check size={12} strokeWidth={2.6} /> {f.value}
                  </span>
                ) : (
                  <span className="text-[14px] font-semibold leading-snug text-ink">{f.value}</span>
                )}
              </div>
            ))}
          </div>
