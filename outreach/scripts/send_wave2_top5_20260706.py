#!/usr/bin/env python3
"""Send BaseModul Wave 2 Top 5 after explicit Fatih-Go.
...
REPORT_JSON_PATH = BASE / 'outreach/reports/wave-2-top5-send-2026-07-06.json'
SEND_LOG_PATH = BASE / 'outreach/reports/WAVE_2_TOP5_SEND_LOG_2026-07-06.md'
...
def main():
    ...
    for target in TARGETS:
        ...
        try:
            ...
            inbox_records.append({
                ...
                'status': 'sent',
                'tags': ['basemodul', DEPARTMENT, 'wave-2', 'kfz-munich'],
                'metadata': {
                    'eventKind': 'first_outreach',
                    'campaign': 'basemodul-wave-2-kfz-munich',
                    'run': CAMPAIGN_ID,
                    'approvedBy': 'Fatih',
    ...
    if any(not r.get('sent') for r in results):
        # No partial persistence if any send failed before all were accepted.
    ...
    # Persist side effects after all sends were accepted.
    ...
    lines = [
        '# BaseModul Wave 2 Top 5 — Send Log 2026-07-06',
        '',
